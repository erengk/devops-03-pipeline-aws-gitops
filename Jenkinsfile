pipeline {
    ...
    stages {
        ...
        stage("Update the Deployment Tags") {
            steps {
                sh """
                   ...
                   sed -i "s|\\(image: *erengk/devops-03-pipeline-aws:\\).*|\\1${IMAGE_TAG}|" deployment.yaml
          ...
          steps {
            withCredentials([usernamePassword(
              ...
              sh '''#!/usr/bin/env bash
                        ...
                        git config user.name  "erengk"
                        git config user.email "gkhnrnkmr@gmail.com"
        ...
        # Token'ı URL'de kullan; Jenkins maskeler
                        git push "https://${GIT_USER}:${GIT_TOKEN}@github.com/erengk/devops-03-pipeline-aws-gitops" HEAD:main
        '''
                        }
                    }
                }
